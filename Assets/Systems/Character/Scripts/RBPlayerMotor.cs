...
public class RBPlayerMotor : LunarScript
{
    ...
    void CheckState()
    {
        ...
        {
            rb.linearDamping = dashing ? dashParams.dashForceDamping : currentAirborneIgnoreDampTime <= 0 ? 0 : moveParams.airborneDamping;
        }
        else
        {
            rb.linearDamping = isSliding ? moveParams.slideDamping : moveParams.walkDamping;
        }
        ...
        else
        {
            ...
            {
                float flatVelSqr = new Vector3(rb.linearVelocity.x, 0, rb.linearVelocity.z).sqrMagnitude;
    ...
    void UpdateSlide()
    {
        if (!InputManager.CrouchInput || rb.linearVelocity.sqrMagnitude < 4f)
        {
            StopSlide();
        }
    }
    ...
    IEnumerator DashCoroutine()
    {
        ...
        dashing = false;
        rb.linearDamping = moveParams.airborneDamping;
    ...
    IEnumerator MantleToPoint(Vector3 point, float speed)
    {
        ...
        if(mantleTime >= 1)
        {
            rb.linearVelocity = transform.rotation * new Vector3(mantleParams.mantleDismountSpeed * InputManager.MoveInput.x, rb.linearVelocity.y, mantleParams.mantleDismountSpeed * InputManager.MoveInput.y);
        }
